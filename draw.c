/* $Id: draw.c,v 1.18 1992/11/29 14:46:57 pturner Exp pturner $
 *
    ...
    double size;
{
	...
	//csto pssetfontsize(size);
    } else if (devtype == 3 || devtype == 4) {
	//csto mifsetfontsize(size);
    } else if (devtype == 7 || devtype == 8) {
	//csto leafsetfontsize(size);
    }
    charsize = size;
    return s;
}
...
//     printf("draw: putlegend %f %f %s \n",x,y,s);
//     printf("draw: putlegend %f %f \n",charsize,devcharsize);
    ...
    setlinestyle(slins);
    setlinewidth(slinw);
//     printf("draw: putlegend %f %i %i %f \n",xtmp + (xlen + 1) * ifudgex, xtmp, xlen, ifudgex);  
    ...
    int device;
{
    ...
    switch (device) {
    case 0:
//csto	retval = xlibinitgraphics(1);
	retval = qtlibinitgraphics(1);
	break;
//     case 1:

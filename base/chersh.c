...
int  slen;
{
   ...
   for (i = 0; i < slen; i++) {
	...
	if (s[i] == '\\') {
	    l=i; k=0;
	    while(s[l] !=' ' && s[l] !='//' && s[l] !='{' && s[l] !='\0') {
	      s_tmp[k] = s[l];
	      k++; l++;
	    }
	    s_tmp[k] = '\0';
	    k=0; 
	    while (strcmp(texsym[k],s_tmp)!=0 && strcmp(texsym[k],"")!=0 ) {
	      k++;
	    }
	    if (strcmp(texsym[k],"") != 0) {
	      s_new=strcat(s_new,xvgrsym[k]);    
	      i=l;
	      j=j+strlen(xvgrsym[k]);
	      s_new[j]='\\'; 
	      s_new[j+1]=(char) ( (int) '0' + curfont );
	      j=j+2;
	    }
	    else {
	      s_new[j]=s[i];

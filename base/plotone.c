/* $Id: plotone.c,v 1.39 1993/03/10 16:32:11 pturner Exp pturner $
 *
...
//     printf("plotone: dolegend\n");
    ...
    j = 0;
    for (i = 0; i < MAXPLOT; i++) {
// 	printf("plotone:dolegend %i %s \n",isactive(gno, i),l.str[i].s);
	if (isactive(gno, i) && (l.str[i].s[0])) {
	    setcolor(l.color);
	    setcharsize(l.charsize);
	    setfont(l.font);
		...
		putlegend(j, 0, l.len, l.vgap, p.symsize,
			  tmpx, tmpy, sy, ly, cy, wy, l.str[i].s, 0,
			  p.symfill, -1, -1, -1);
		break;
	    case BAR:
	    case STACKEDBAR:
		putlegend(j, 0, l.len, l.vgap, p.symsize,
			  tmpx, tmpy, sy, ly, cy, wy, l.str[i].s, 1,
    ...
    world w;
{
    ...
    if (f.type == 0) {
        //printf("boxplot:rect %f %f %f %f\n",w.xg1, w.yg1, w.xg2, w.yg2);
	rect(w.xg1, w.yg1, w.xg2, w.yg2);
    } else {
        //printf("boxplot:move %f %f %f %f\n",w.xg1, w.yg1, w.xg2, w.yg2);
 ...
 */
void drawsetxy(plotarr p, int i)
{

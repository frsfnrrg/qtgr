/* $Id: drawticks.c,v 1.26 1993/03/20 16:20:35 pturner Exp pturner $
 *
...
void drawxticklabels(int gno, int caxis)
{
    ...
    for (i = 0; i < nticks; i += t.tl_skip) {
        ...
        if (t.tl_op == BOTTOM || t.tl_op == BOTH) {
            (*devwritestr) ((*devconvx) (x), (*devconvy) (bot - delb) - yoff, tang, s, bjust, 1);
        }

        if (t.tl_op == TOP || t.tl_op == BOTH) {
            (*devwritestr) ((*devconvx) (x), (*devconvy) (top + delt) + yoff, tang, s, tjust, 0);
        }
    }
...
void drawyticklabels(int gno, int caxis)
{
    ...
    for (i = 0; i < nticks; i += t.tl_skip) {
        ...
        if (t.tl_op == LEFT || t.tl_op == BOTH) {
            (*devwritestr) ((*devconvx) (bot - dell) - xoff, (*devconvy) (y), tang, s, ljust, 1);
        }
        if (t.tl_op == RIGHT || t.tl_op == BOTH) {
             (*devwritestr) ((*devconvx) (top + delr) + xoff, (*devconvy) (y), tang, s, rjust, 0);
        }
    }
    /* TODO axis label layout */
    if (t.label.s[0]) {

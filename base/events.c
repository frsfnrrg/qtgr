...
//	set_wait_cursor();//frsfnrrg
	initgraphics(tdevice);
//	if (drawimage_flag) {//frsfnrrg
//	    drawimage();
//	}
...
//	draw_focus(cg);//frsfnrrg
	leavegraphics();
	force_redraw = FALSE;
//	unset_wait_cursor();//frsfnrrg
    }
}
...
//             stringextentx(charsize * xlibcharsize, stmp );
//        xtx = -(int) tx *co;
//        ytx = -(int) tx *si;
//        xhgt = (int) -hgt * si;
//        yhgt = (int) hgt *co;
...
//		         (newg = nextcontained(cg, wx, wy)) != cg &&
...
//		    set_write_mode(0);
//		    draw_string(-2, no);
//			wy = (double)stringextenty(charsize * xlibcharsize, pstr[no].s)/
//														(double)win_h;
//		    if (pstr[no].loctype == COORD_VIEW) {
//				wx1 = xconv(wx1);
//				wy1 = yconv(wy1) - wy/2.;

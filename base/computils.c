/* $Id: computils.c,v 1.19 1993/02/06 16:59:39 pturner Exp pturner $
 *
 ...
 * splines
 */
void do_spline(set, start, stop, n)
    int set, n;
    double start, stop;
{
    int i, splineset, len, ier;
    double u, delx, *x, *y, *b, *c, *d, *xtmp, *ytmp;
    double xmin, xmax, ymin, ymax, seval();
    extern double result;
...
void do_spline_command(set, start, stop, n)
    int set, n;
    double start, stop;
{
    ...
    double bins, xmin, xmax;
{
    ...
    //frsfnrrg updatesymbols(cg, hset);
